...
namespace SidesMpcConfigurationUtility.Classes
{
    ...
    public class SettingsOperations
    {
        ...
        public static (bool success, Exception exception, List<SettingItem> items, List<Type> types)  ReadSettings(SidesEnvironment environment = SidesEnvironment.Development)
        {
            ...
            try
            {
                ...
                typeList = list.Select(x => x.Type).Distinct().ToList();



                return (true, null, list, typeList);
            }
            catch (Exception ex)
            {
